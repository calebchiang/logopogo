...
export default function GenerateLogo() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const json: GenResponse | { error: string } = await res.json()
      if (!res.ok) throw new Error(('error' in json && json.error) || 'Generation failed')
      setImages((json as GenResponse).images)
    } catch (err: any) {
    ...
    <div className="mx-auto max-w-5xl px-6 py-16">
      ...
      <div className="p-2 md:p-8 bg-[var(--background)]">
        ...
        >
            ...
            className="w-full sm:w-[420px] flex-none h-12 rounded-xl border border-zinc-800 bg-zinc-900/40 px-4
                 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-700"
            ...
            type="submit"
            disabled={!brand.trim()}
            className="h-12 rounded-xl px-5 bg-indigo-600 text-white border border-zinc-800
                        hover:bg-zinc-800"
            >
            ✨ Make logo
            </button>
